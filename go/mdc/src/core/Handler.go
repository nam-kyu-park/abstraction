...
func (this *HandlerManager) recv() {
	this.logger.Info("running callback recv function.")
	defer this.Close()
	for {
		...
		data, err := packet.Decode(&buffer)
		if err != nil {
			this.logger.Warn("decoding packet failed. : %s (%d)", err.Error(), n)
			this.logger.Warn("decoding packet failed debug: \n%s", packet.Debug(buffer))
			this.logger.Warn("%+v", data)
			continue
		}

		t := int(data.Service)
		sv := this.serviceMap[t]
		if sv == nil {
