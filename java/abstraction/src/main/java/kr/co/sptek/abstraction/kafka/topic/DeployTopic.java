...
public class DeployTopic  extends Object {

    private static final Logger logger = LogManager.getLogger(DeployTopic.class);

    public String company_id;
    public List<String> device_id;
    public List<String> mdc_id;
    ...
    public String toString() {
        return "DeployTopic [ company_id=" + company_id + ", mdc_id=" + mdc_id  + ", device_id=" + device_id
                + ", version=" + version + ", status=" + status + ", timestamp=" + timestamp + " ]";
    }

    public static DeployTopic deserializeJSON(final String topic) throws IOException {
    ...
    public static DeployTopic deserializeJSON(final JSONObject topic) throws IOException {
        JSONObject obj = (JSONObject)topic.get(DcimTopicId.TOPIC_DEPLOY);
        if(obj != null) {
            logger.info("call deserializeJSON: JSONObject topic");
            DeployTopic data = new DeployTopic();
            data.company_id = (String)obj.get("company_id");
            data.device_id = deserializeJSONArray(obj.get("device_id"));
            data.mdc_id = deserializeJSONArray(obj.get("mdc_id"));
            data.version = (String)obj.get("version");
    ...
    public static String serializeJSON(final DeployTopic data) throws IOException {
        return DeployTopic.serializeJSONObject(data).toString();
    }

    public String serializeJSON() throws IOException {
        return DeployTopic.serializeJSONObject(this).toString();
    }
}